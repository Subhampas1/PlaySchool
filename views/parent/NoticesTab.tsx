...
interface NoticesTabProps {
  ...
  userId: string;
  onNavigateToFees?: () => void; // Optional callback to navigate to Fees tab
}
        ...
        <div className="grid grid-cols-1 gap-4">
          {notices
            ...
            .map((notice) => (
              <Card
                key={notice.id}
                className={`p-5 border-l-4 cursor-pointer hover:shadow-lg transition-all transform hover:-translate-y-0.5 ${
                  priorityColors[notice.priority].border
                } ${priorityColors[notice.priority].bg}`}
              >
                <div onClick={() => handleNoticeClick(notice)}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {!isRead(notice) &&
                        notice.id !== "virtual-fee-reminder" && (
        ...
        {selectedNotice && (
          <div className="space-y-4">
              ...
              {selectedNotice.category && (
                <span
                  className={`px-3 py-1 text-xs font-bold rounded-full uppercase tracking-wider ${
                    categoryConfig[selectedNotice.category]?.bg
                  } ${categoryConfig[selectedNotice.category]?.color}`}
                >
                  {selectedNotice.category}
                </span>
              )}
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
              {selectedNotice.category === "Fee Reminder" &&
              onNavigateToFees ? (
                <button
                ...
                <button
                  disabled
                  className="w-full flex items-center justify-center gap-2 py-3 bg-gray-100 text-gray-500 font-bold rounded-xl dark:bg-gray-700 dark:text-gray-400 cursor-not-allowed"
                >
